...
import { getInfo, getProjectById, getUserById, getUsers} from '../redux/actions'
 ...
 const edition = ()=>{
  ...
  return (
    <div>
      ...
      <GenericButton buttonText={'Agregar'} onClick={()=>{navigate('/create')}}  disabled= {!id} className={bstyle.button}/>
      <GenericButton buttonText={'Eliminar'} onClick={delPage}  disabled= {!id} className={bstyle.button}/>
      <GenericButton buttonText={'Logout'} onClick={()=>{logout(); navigate(`/`)}}  disabled= {id} className={bstyle.button}/>
      ...
      <>{(view === 'user')?
        ...
        {!id?
         ...
         {modal? 
          <comp.CreateItem pageId={id} onClose3={onClose3}/> : null
         }
         {showDel?
         <comp.DeleteItem pageId={id} response = {response} closeDel= {closeDel}/> : null
         }</>
         }</>}</>: 
         <comp.Loading/>}
       <comp.Footer/>
